...
class Sistema {
    ...
    salvar() {
        ...
        fs.writeFileSync('passagens.json', JSON.stringify(dados, null, 2));
    }

    carregar() {
        if (!fs.existsSync('passagens.json')) return;
    ...
    listarTrechos() {
        ...
        if (this.trechos.length === 0) {
            ...
            this.trechos.forEach((trecho, index) => {
                ...
                console.log(`   Companhia: ${trecho.companhia}`);
    ...
    switch (opcao) {
        ...
        case 2:
            ...
            switch (opcaoTrecho) {
                case 1:
                    if (sistema.companhias.length === 0) {
                        console.log("\nCadastre uma companhia antes de adicionar trechos.");
                    } else {
                        sistema.listarCompanhias();
